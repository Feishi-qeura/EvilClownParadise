...
- **角色独占的美术资产**自包含在 `Characters/<角色>/` 里（模型、骨架、材质、贴图、该角色的动画序列）
- **跨角色共享的系统资产**放顶层同级目录（共享 ABP 在 `Animations/`）
- 玩法蓝图与 AI 数据按职责单独分层（`Blueprints/`、`AI/`、`Data/`）

判据：**这东西会跟着角色一起退役吗？** 会 → 放角色目录；不会（多个角色共用、角色换了它还在）
→ 放共享层。

```
_Game/
  Animations/                   ← 共享动画层（跨角色复用，与角色目录平级）
    ABP_TestRobot               ← 共享动画蓝图，就地不动
    （将来跨角色共用的重定向动画序列也放这里）
...
**动画序列随角色，动画蓝图进共享层**——这两类东西的寿命不一样：

- **动画序列**（`A_*`）绑定骨架，`A_Run_F_Masc` 只能给 `SKEL_TestRobot` 用，角色没了它就没用了
  → 随角色。
- **动画蓝图**（`ABP_*`）是**动画系统**，数量有限（几个）且跨角色复用：玩家和小丑各有模型时，
  很可能共用同一个 ABP 去做状态机与重定向。它不随任何单个角色退役 → 放共享层。
  若塞进某个角色目录，其他角色引用它很别扭，且该角色退役时会误删共享系统。

当动画序列变多（预计 100+）时，在角色的 `Animations/` 内按用途再分子目录，直接复用
原沙盒那套已经被验证好用的分类：
...
> **关于跨角色共用动画序列**：将来若希望多个类人角色共用一套移动动画，走引擎的骨骼重定向
> （Retarget），共享的那套序列放顶层 `Animations/`（与共享 ABP 同层）。**现在不要提前建空目录**——
> 空目录 git 不跟踪，克隆后不存在，反而让结构和文档不一致。

> **共享 ABP 的命名**：名字应体现它服务的是哪套动画系统，而不是某个角色。现在只有一个角色，
...
**本次要动的清单**：6 个 `Characters/*`（含 3 个改名）+ 4 个动画序列进 `Characters/TestRobot/Animations/`。
`Animations/ABP_TestRobot` **不动**（它已经在共享层）。
