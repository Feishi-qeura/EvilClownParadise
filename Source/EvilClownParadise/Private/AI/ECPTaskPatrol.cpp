...
EStateTreeRunStatus FECPTaskPatrol::EnterState(FStateTreeExecutionContext& Context,
...
{
	AECPMonsterBase* Monster = ECPTree::GetMonster(Context);
	AAIController* AIController = Monster ? Monster->GetController<AAIController>() : nullptr;
	...
	FInstanceDataType& InstanceData = Context.GetInstanceData(*this);
	const float TimeNow = Monster->GetWorld()->GetTimeSeconds();
	...
	if (InstanceData.bHeadingToTarget)
	{
		...
		{
			InstanceData.bHeadingToTarget = false;
			InstanceData.NextPickTime = TimeNow + PauseDuration; // 到点后停顿
		}
	}
	else if (TimeNow >= InstanceData.NextPickTime)
	{
		...
		if (UNavigationSystemV1* NavSys = FNavigationSystem::GetCurrent<UNavigationSystemV1>(Monster->GetWorld()))
		{
			...
			if (NavSys->GetRandomReachablePointInRadius(Monster->GetPatrolOrigin(), Monster->PatrolRadius, NavLoc))
			{
				...
				const EPathFollowingRequestResult::Type MoveResult =
				    AIController->MoveToLocation(NavLoc.Location, AcceptanceRadius);
...
void FECPTaskPatrol::ExitState(FStateTreeExecutionContext& Context, const FStateTreeTransitionResult& Transition) const
{
	...
	{
		if (AAIController* AIController = Monster->GetController<AAIController>())
		{
