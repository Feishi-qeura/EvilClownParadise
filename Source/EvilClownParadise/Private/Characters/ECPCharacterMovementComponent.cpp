...
void UECPCharacterMovementComponent::UpdateCharacterStateBeforeMovement(float DeltaSeconds)
{
	...
	{
		bLandAllowsJump = (LandKind == EECPLandKind::Soft)
			|| (LandLockRemaining <= GetLandLockDuration() * LandCancelWindowRatio);
	}
	
	const bool bCanJumpNow = JumpBufferRemaining > 0.f && (IsMovingOnGround() || CoyoteRemaining > 0.f) && !IsCrouching() && bLandAllowsJump;
	...
	if (LandLockRemaining > 0.0f
		...
		&& bHasMoveInput)
	{
		const bool bCanCancelMove = (LandKind == EECPLandKind::Soft)
			|| (LandLockRemaining <= GetLandLockDuration() * LandCancelWindowRatio);
...
void UECPCharacterMovementComponent::OnMovementModeChanged(EMovementMode PreviousMovementMode, uint8 PreviousCustomMode)
{
	...
	if (PreviousMovementMode == EMovementMode::MOVE_Falling && (MovementMode == MOVE_Walking || MovementMode == MOVE_NavWalking))
	{
		ClassifyLanding();
	}
...
float UECPCharacterMovementComponent::GetLandControl() const
{
	...
	switch (LandKind)
	{
		case EECPLandKind::Soft : return LandSoftControl;
		case EECPLandKind::Medium: return LandMediumControl;
		case EECPLandKind::Hard : return LandHardControl;
		default: return 1.0f;
	}
}
...
float UECPCharacterMovementComponent::GetLandLockDuration() const
{
	switch (LandKind)
	{
		case EECPLandKind::Soft : return SoftLandTime;
		case EECPLandKind::Medium: return MediumLandTime;
		case EECPLandKind::Hard : return HardLandTime;
		default: return 0.0f;
	}
}
